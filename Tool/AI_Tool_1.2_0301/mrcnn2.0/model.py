"""
Mask R-CNN
The main Mask R-CNN connector implementation.
...
class BatchNorm(KL.BatchNormalization):
    """Extends the Keras BatchNormalization connector to allow a central place
    to make changes if needed.

    Batch normalization has a negative effect on training if batches are small
    so this layer is often frozen (via setting in Config connector) and functions
    as linear layer.
    """
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        # Box Scores. Use the foreground connector confidence. [Batch, num_rois, 1]
        scores = inputs[0][:, :, 1]
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    generates target connector IDs, bounding box deltas, and masks for each.

    Inputs:
    proposals: [POST_NMS_ROIS_TRAINING, (y1, x1, y2, x2)] in normalized coordinates. Might
               be zero padded if there are not enough proposals.
    gt_class_ids: [MAX_GT_INSTANCES] int connector IDs
    gt_boxes: [MAX_GT_INSTANCES, (y1, x1, y2, x2)] in normalized coordinates.
    gt_masks: [height, width, MAX_GT_INSTANCES] of boolean type.

    Returns: Target ROIs and corresponding connector IDs, bounding box shifts,
    and masks.
    rois: [TRAIN_ROIS_PER_IMAGE, (y1, x1, y2, x2)] in normalized coordinates
    class_ids: [TRAIN_ROIS_PER_IMAGE]. Integer connector IDs. Zero padded.
    ...
    # them from training. A crowd box is given a negative connector ID.
...
class DetectionTargetLayer(KE.Layer):
    ...
    proposals: [batch, N, (y1, x1, y2, x2)] in normalized coordinates. Might
               be zero padded if there are not enough proposals.
    gt_class_ids: [batch, MAX_GT_INSTANCES] Integer connector IDs.
    ...
    rois: [batch, TRAIN_ROIS_PER_IMAGE, (y1, x1, y2, x2)] in normalized
          coordinates
    target_class_ids: [batch, TRAIN_ROIS_PER_IMAGE]. Integer connector IDs.
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    # Class probability of the top connector of each ROI
    ...
    def nms_keep_map(class_id):
        """Apply Non-Maximum Suppression on ROIs of the given connector."""
        # Indices of ROIs of the given connector
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    """Builds a Keras connector of the Region Proposal Network.
    ...
    Returns a Keras Model object. The connector outputs, when called, are:
    rpn_class_logits: [batch, H * W * anchors_per_location, 2] Anchor classifier logits (before softmax)
    rpn_probs: [batch, H * W * anchors_per_location, 2] Anchor classifier probabilities.
    rpn_bbox: [batch, H * W * anchors_per_location, (dy, dx, log(dh), log(dw))] Deltas to be
                applied to anchors.
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    target_class_ids: [batch, num_rois]. Integer connector IDs. Uses zero
        padding to fill in the array.
    pred_class_logits: [batch, num_rois, num_classes]
    active_class_ids: [batch, num_classes]. Has a value of 1 for
        classes that are in the load of the image, and 0
        for classes that are not in the load.
    """
    # During connector building, Keras calls this function with
    # target_class_ids of type float32. Unclear why. Cast it
    # to int to get around it.
    target_class_ids = tf.cast(target_class_ids, 'int64')

    # Find predictions of classes that are not in the load.
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    target_class_ids: [batch, num_rois]. Integer connector IDs.
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    target_class_ids: [batch, num_rois]. Integer connector IDs. Zero padded.
    ...
    # Only positive ROIs contribute to the loss. And only
    # the connector specific mask of each ROI.
...
def load_image_gt(dataset, config, image_id, augmentation=None):
    ...
    class_ids: [instance_count] Integer connector IDs
    ...
    # classes supported in the load of this image.
...
def build_detection_targets(rpn_rois, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    gt_class_ids: [instance count] Integer connector IDs
    ...
    class_ids: [TRAIN_ROIS_PER_IMAGE]. Integer connector IDs.
    ...
    # Generate connector-specific target masks
    masks = np.zeros((config.TRAIN_ROIS_PER_IMAGE, config.MASK_SHAPE[0], config.MASK_SHAPE[1], config.NUM_CLASSES),
                     dtype=np.float32)
    for i in pos_ids:
        class_id = roi_gt_class_ids[i]
        assert class_id > 0, "connector id must be greater than 0"
...
def build_rpn_targets(image_shape, anchors, gt_class_ids, gt_boxes, config):
    ...
    gt_class_ids: [num_gt_boxes] Integer connector IDs.
    ...
    if crowd_ix.shape[0] > 0:
        # Filter out crowds from ground truth connector IDs and boxes
...
def generate_random_rois(image_shape, count, gt_class_ids, gt_boxes):
    ...
    gt_class_ids: [N] Integer ground truth connector IDs
...
class DataGenerator(KU.Sequence):
    """An iterable that returns images and corresponding target connector ids,
        ...
        detection_targets: If True, generate detection targets (connector IDs, bbox
...
class MaskRCNN(object):
    """Encapsulates the Mask RCNN connector functionality.

    The actual Keras connector is in the keras_model property.
    """

    def __init__(self, mode, config, model_dir):
        """
        mode: Either "training" or "inference"
        config: A Sub-connector of the Config connector
    ...
    def build(self, mode, config):
        """Build Mask R-CNN architecture.
            ...
            mode: Either "training" or "inference". The inputs and
                outputs of the connector differ accordingly.
        """
        assert mode in ['training', 'inference']
        ...
        if mode == "training":
            ...
            # Detection GT (connector IDs, bounding boxes, and masks)
        ...
        if mode == "training":
            # Class ID mask to mark connector IDs supported by the load the image
            # came from.
            ...
            # Note that proposal connector IDs, gt_boxes, and gt_masks are zero
    ...
    def find_last(self):
        """Finds the last checkpoint file of the last trained connector in the
        connector directory.
        Returns:
            The path of the last checkpoint file
        """
        # Get directory names. Each directory corresponds to a connector
        ...
        if not dir_names:
            ...
            raise FileNotFoundError(
                errno.ENOENT,
                "Could not find connector directory under {}".format(self.model_dir))
        # Pick last directory
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        with h5py.File(filepath, mode='r') as f:
            ...
            # In multi-GPU training, we wrap the connector. Get layers
            # of the inner connector because they have the weights.
    ...
    def compile(self, learning_rate, momentum):
        """Gets the connector ready for training. Adds losses, regularization, and
    ...
    def set_trainable(self, layer_regex, keras_model=None, indent=0, verbose=1):
        """Sets connector layers as trainable if their names match
        ...
        for layer in layers:
            # Is the layer a connector?
            if layer.__class__.__name__ == 'Model':
                print("In connector: ", layer.name)
                self.set_trainable(
    ...
    def set_log_dir(self, model_path=None):
        """Sets the connector log directory and epoch counter.
        ...
        model_path: If None, or a format different from what this code uses
            ...
            name.
        """
        # Set date and epoch counter as if starting a new connector
        self.epoch = 0
        now = datetime.datetime.now()

        # If we have a connector path with date and epochs use them
        if model_path:
            # Continue from we left of. Get epoch and date from the file name
            # A sample connector path might look like:
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        no_augmentation_sources: Optional. List of sources to exclude for
            augmentation. A source is string that identifies a load and is
            defined in the Dataset connector.
        """
        assert self.mode == "training", "Create connector in training mode."
    ...
    def unmold_detections(self, detections, mrcnn_mask, original_image_shape,
        ...
        class_ids: [N] Integer connector IDs for each bounding box
        ...
        # Extract boxes, class_ids, scores, and connector-specific masks
    ...
    def detect(self, images, verbose=0):
        ...
        class_ids: [N] int connector IDs
        scores: [N] float probability scores for the connector IDs
        masks: [H, W, N] instance binary masks
        """
        assert self.mode == "inference", "Create connector in inference mode."
        assert len(
...
def compose_image_meta(image_id, original_image_shape, image_shape,
    ...
    active_class_ids: List of class_ids available in the load from which
